...
class NuovoMotociclo extends JFrame {

	private JTextField textField;
	private JTextField textField_1;

	NuovoMotociclo(String targa) {
		setTitle("Aggiungi motociclo");
		...
		JLabel lblNumeroDiPorte = new JLabel("Numero di ruote");
		...
		gbc_lblNumeroDiPorte.gridy = 3;
		contentPane.add(lblNumeroDiPorte, gbc_lblNumeroDiPorte);

		JSpinner spinner_1 = new JSpinner();
		spinner_1.setModel(new SpinnerNumberModel(2, 1, null, 1));
		...
		JLabel lblNumeroDiPost = new JLabel("Numero di passeggeri");
		...
		gbc_lblNumeroDiPost.gridy = 4;
		contentPane.add(lblNumeroDiPost, gbc_lblNumeroDiPost);

		JSpinner spinner_2 = new JSpinner();
		spinner_2.setModel(new SpinnerNumberModel(2, 1, null, 1));
		...
		btnAggiungi.addActionListener((e) -> {
			...
			int numeroRuote = (int) spinner_1.getValue();
			int numeroPasseggeri = (int) spinner_2.getValue();
			if (marca.isEmpty() || modello.isEmpty()) {
				JOptionPane.showMessageDialog(null, "Devi compilare tutti i campi.", "Errore",
						JOptionPane.ERROR_MESSAGE);
			} else {
				try {
					ArchivioAutomezzi.inserisci(new Motociclo(marca, modello, targa, anno, numeroRuote,
							numeroPasseggeri));
					JOptionPane.showMessageDialog(null, "Motociclo caricato", "Inserimento avvenuto",
							JOptionPane.PLAIN_MESSAGE);
					dispose();
				} catch (TargaErrataException ex) {
					JOptionPane.showMessageDialog(null, "Impossibile inserire la moto.", "Inserimento fallito",
							JOptionPane.ERROR_MESSAGE);
				}
			}
		});
